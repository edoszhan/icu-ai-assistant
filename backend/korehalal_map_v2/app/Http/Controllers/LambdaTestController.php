    ...
    public function checkLambdaConnection(Request $request)
    {
        ...
        try {
            $response = Http::withOptions([
                'stream' => true, // Enable streaming request to Lambda
            ])->post($lambdaUrl, [
                'prompt' => $prompt,
            ]);
            ...
            return response()->stream(function () use ($response) {
                ...
                while (!$stream->eof()) {
                    $chunk = trim($stream->read(4096)); // Read chunks of data

                    if (!empty($chunk)) {
                        // Decode JSON chunk safely
                        $jsonChunk = json_decode($chunk, true);
                        if (isset($jsonChunk['chunks'])) {
                            foreach ($jsonChunk['chunks'] as $textChunk) {
                                echo "data: " . $textChunk . "\n\n";
                                ob_flush();
                                flush();
                            }
                    ...
                    usleep(50000); // Small delay to mimic streaming effect
                }
